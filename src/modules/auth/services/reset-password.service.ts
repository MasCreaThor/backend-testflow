...
import { SendPasswordResetEmailService } from '../../email/services/send-password-reset-email.service';
...
export class ResetPasswordService {
  constructor(
    ...
    this.logger.setContext(ResetPasswordService.name);
  }
  ...
  async requestPasswordReset(dto: ResetPasswordRequestDto): Promise<{ message: string }> {
    ...
    const emailSent = await this.sendPasswordResetEmailService.execute(
      user.email,
      resetToken,
      fullName,
    );
